...
CEGameView::CEGameView(CEGameState* gameState, QWidget* parent)
	:QFrame{parent}, gameState{gameState}
{
...
void CEGameView::paintEvent(QPaintEvent* event) {
	...
	for (int strIdx = 0; strIdx < gameState->getByteStr().size(); ++strIdx) {
		if (gameState->getByteStr()[strIdx] == '1') {
			QPen pen(Qt::red);
			pen.setWidth(3);
			painter.setPen(pen);
			painter.drawLine((strIdx * 20) + 20, 100, (strIdx * 20) + 20, 250);
		}
		else if (gameState->getByteStr()[strIdx] == '0') {
			QPen pen(Qt::blue);
			pen.setWidth(3);
			painter.setPen(pen);
			painter.drawLine((strIdx * 20) + 20, 100, (strIdx * 20) + 20, 250);
